...
\subsection{Statements :}\label{sec:Statements} 
  ...
  \centering
    ...
    \hline
    Cost Statement & Ai costs ki (if Ai causes $\overline{f}$ if $\overline{g1}$,....,$\overline{gn}$), ki $\in$ K for i = 1,...,n & If the action Ai is performed in any state satisfying $\overline{g1}$, ...,$\overline{gk}$ , then change in state results in fixed cost ki. \\
    \hline
    \end{tabular}
...
\item A cost transition function is a mapping $\Gamma$ : Ac $\times \sum \rightarrow$ K.For any $\sigma\in\sum$ and for any action Ai $\in$ Ac where i = 1,...,n, $\Gamma$(Ai, $\sigma$) is the fixed cost ki corresponding to the action Ai, where ki $\in$ K and i=1,...,n, resulting from performing the action Ai in the state $\sigma$. Also $\Gamma$(Ai,$\sigma$) will results in 0 cost if there is no change in state. 
...
\item if s is of the form A1,...,An costs k1,...,kn respectively where ki $\in$ K, Ai $\in$ Ac and i = 1,...n, then every $\sigma \in \sum$ such that $\sigma \vDash \overline{gi}$, i = 1,...,k, $\Gamma$(Ai,$\sigma$) $\vDash$ ki.
\end{enumerate}
...
\begin{itemize}
	...
	\item if $\Psi$(Ai,$\sigma$) $\nvDash$ $\sigma$ then $\Gamma$(Ai,$\sigma$) = ki.
\end{itemize}
...
\item if $\Psi$(Ai,$\sigma$) = $\sigma$ then $\Gamma$(Ai,$\sigma$) = 0.
\end{itemize}
...
\begin{itemize}
\item{Query set Q1:\\
necessary $\sigma$ after A1,...,Ai on $\sigma_{0}$ \\
possibly $\sigma$ after A1,....,Ai on $\sigma_{0}$ \\
\\
The first statement says that state $\sigma$ will always occurs after executing the program on initial state. \\
The second statement says that state $\sigma$ may occurs after executing the program on initial state. \\}
\\
\item{Query set Q2: \\
necessary cost C for A1,....,Ai\\
sufficient cost C for A1,....,Ai\\
\\
The first statement says that cost C is necessary to execute the program\\
The second statement says that cost C is sufficient to execute the program\\}
\end{itemize}
\subsection{Semantics}
\begin{itemize}
	\item{The query set Q1 is defined as Q1 = ($\sigma$,P,$\sigma_{0}$) where $\sigma_{0}$ is the initial state, $\sigma$ is the resulting state from the execution of program P and P is a sequence defined as P = (A1,...,An), n $\geq$ 0 of actions and $\sigma$, $\sigma_{0}$ $\in$ $\sum$ }
	\item{The Query set Q2 is defined as Q2=(C,P) where C is the cost to execute the program P and P is a sequence defined as P=(A1,....,An), n $\geq$ 0 of actions}
\end{itemize}
	...
	\\
	$\Psi$(buy, $\sigma_{3}$)=$\sigma_{0}$\\ 
	$\Psi$(travel, $\sigma_{3}$)=$\sigma_{2}$\\
	\(\Gamma\)(buy, $\sigma_{3}$) = 40\\
	...
	necessary $\sigma_{3}$ for travel, travel, buy on $\sigma_{0}$\\
	possibly $\sigma_{1}$ for buy, buy, travel on $\sigma_{0}$\\
	necessary $\sigma_{2}$ for travel, travel, buy, travel on $\sigma_{0}$\\
	\\
	necessary cost 140 for travel, travel, buy\\
	sufficient cost 100 for buy, travel, buy\\
	necessary cost 190 for travel, travel, buy, travel\\
	...
	\end{figure}
	\subsubsection{Queries}
	necessary $\sigma_{1}$ for order, buy, order on $\sigma_{0}$\\
	possibly $\sigma_{1}$ for buy, buy, order on $\sigma_{0}$\\
	necessary $\sigma_{2}$ for order, buy on $\sigma_{0}$\\
	\\
	necessary cost 300 for order, buy, order\\
	sufficient cost 60 for buy, buy, order\\
	necessary cost 250 for order, buy\\
	...
	\end{figure}
	\subsubsection{Queries}
	necessary $\sigma_{2}$ for cook, eat, cook on $\sigma_{0}$\\
	possibly $\sigma_{3}$ for play, cook, play, eat on $\sigma_{0}$\\
	necessary $\sigma_{1}$ for cook, eat, play, cook on $\sigma_{0}$\\
	\\
	necessary cost 35 for cook, eat, cook\\
	sufficient cost 25 for play, cook, play, eat\\
	necessary cost 55 for cook, eat, play, cook\\
	\newpage
	\section{Appendix}	
	\begin{appendix}
		\listoffigures
